...
const CartPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCartsAndOrders = async () => {
      try {
        ...
        const orderResponse = await axios.get(`${API_URL}/shop/user/orders`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleAddProduct = async (cartId: number, product: Product) => {
    ...
    try {
      await axios.put(
        `${API_URL}/shop/user/cart/${cartId}/${product.id}`,
        {
  ...
  const handleRemoveProduct = async (cartId: number, productId: number, currentQuantity: number) => {
    try {
      if (currentQuantity > 1) {
        ...
        await axios.put(
          `${API_URL}/shop/user/cart/${cartId}/${productId}`,
          {
        ...
        await axios.delete(`${API_URL}/shop/user/cart/Item/${cartId}/${productId}`, {
          headers: { Authorization: `Bearer ${token}` },
